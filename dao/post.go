...
func UpdateViewCountBypId(pid, viecount int) error {
	_, err := DB.Exec("update blog_post set view_count=? where pid=?", viecount+1, pid)
	if err != nil {
		log.Println(err)
		return err
	}
...
func GetPostPage(page, pageSize int) ([]models.Post, error) {
	...
	rows, err := DB.Query("select * from blog_post order by pid desc limit ?,?", page, pageSize)
	if err != nil {
		return nil, err
	}
...
func GetPostPageByCategoryId(cId, page, pageSize int) ([]models.Post, error) {
	page = (page - 1) * pageSize
	rows, err := DB.Query("select * from blog_post where category_id= ? order by pid desc limit ?,?", cId, page, pageSize)
	if err != nil {
		return nil, err
	}
	var posts []models.Post
...
func GetPostSearch(condtion string) ([]models.Post, error) {
...
func GetPostAll() ([]models.Post, error) {
	...
	rows, err := DB.Query("select * from blog_post order by pid")
	if err != nil {
		return nil, err
	}
	var posts []models.Post
